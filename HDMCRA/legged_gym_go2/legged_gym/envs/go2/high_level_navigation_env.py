...
class HighLevelNavigationEnv:
    ...
    - Observations: [cos_heading, sin_heading, body vx, body vy, yaw_rate, dist_to_target, target_dir_body]
      plus optional lidar bins and energy state
    ...
    def __init__(self, base_env: GO2Robot, cfg):
        ...
        # Energy adds 1 to observation dimension
        self._energy_obs_dim = 1
        self.num_high_level_obs = self._base_obs_dim + target_feature_dim + lidar_feature_dim + self._energy_obs_dim
        ...
        # Energy state (EC-EFPPO)
        self.min_energy = getattr(cfg, 'min_energy', -400.0)
        self.max_energy = getattr(cfg, 'max_energy', 800.0)
    ...
    def reset(self):
        ...
        Returns:
            high_level_obs: [num_envs, obs_dim] high-level observations (including energy)
            g_values: [num_envs] initial g-function values
            h_values: [num_envs] initial h-function values
            energy: [num_envs] initial energy values
        """
        # 重置底层环境
    ...
    def _compute_high_level_observations(self):
        """Compute high-level observations including target/unsafe distances and energy."""
    ...
    def get_energy(self):
        """返回当前能量预算 [num_envs]"""
        return self.energy.clone()

    def get_energy_consumption(self):
        """返回当前步能量消耗 [num_envs]"""
        return self.energy_consumption.clone()
...
class HighLevelNavigationConfig:
    ...
    def __init__(self):
        ...
        # Energy state parameters (EC-EFPPO)
